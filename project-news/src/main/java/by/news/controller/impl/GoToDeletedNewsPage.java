...
public class GoToDeletedNewsPage implements Command {
	...
	private static final String URL = "frontController?command=go_to_deleted_news_page";
	...
	private static final int NEWS_STATUS_DELETED = NewsStatus.DELETED.ordinal();
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			long numberOfNews = newsService.getNumberOfNews(NEWS_STATUS_DELETED, newsPattern);
			numberOfPages = (int) Math.ceil(numberOfNews * 1.0 / limitOnPage);
			listOfNews = newsService.getListOfNews((page - 1) * limitOnPage, limitOnPage, NEWS_STATUS_DELETED,
					newsPattern);
