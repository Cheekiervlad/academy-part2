...
public class GoToAuthUserPage implements Command {
	...
	private static final String URL = "frontController?command=go_to_auth_user_page";
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			long numberOfNews = newsService.getNumberOfNews(NEWS_STATUS_APPROVED, newsPattern);
			numberOfPages = (int) Math.ceil(numberOfNews * 1.0 / limitOnPage);
			listOfNews = newsService.getListOfNews((page - 1) * limitOnPage, limitOnPage, NEWS_STATUS_APPROVED,
					newsPattern);
